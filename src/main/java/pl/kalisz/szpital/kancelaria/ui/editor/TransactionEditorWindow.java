...
public class TransactionEditorWindow extends Window {
  ...
  private final OptionGroup typOptionGroup = new OptionGroup();
  ...
  private String getKomorka() {
    ...
    LinkedHashSet<KomorkaOrganizacyjna> set =
        (LinkedHashSet<KomorkaOrganizacyjna>) komorkaTokenField.getValue();
    if (set == null) {
  ...
  private boolean walidacjaKomorkaOrg() {
    ...
    // logger.trace(komorkaStr);
    if (komorkaStr.equals("")) {
      return false;
    }
    // String[] org = komorkaStr.split(" ");
    // logger.trace(Arrays.toString(org));
    // for (String o : org) {
    // if (o.equals("")) {
    // continue;
    // }
    // if (!KomorkaOrganizacyjna.contains(o)) {
    // return false;
    // }
    // }
    return true;
