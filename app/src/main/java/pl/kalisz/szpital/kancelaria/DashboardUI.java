...
public class DashboardUI extends UI {
	...
	private void buildLoginView() {
		...
		signin.addClickListener(new ClickListener() {
			...
			public void buttonClick(final ClickEvent event) {
				...
				uc.addContainerFilter(new Compare.Equal(User.LOGIN, username.getValue()));
				Iterator<Object> ids = (Iterator<Object>) uc.getItemIds().iterator();
				User user = null;
				if (ids.hasNext()) {
					Integer id = (Integer) ids.next();
					user = uc.getItem(id).getEntity();
					System.out.println("User exists " + username.getValue());
				} else {
					System.out.println("NO USER: " + username.getValue());
				}
	...
	private void buildMainView() {
		...
		nav.addView("/zdarzenia", TransactionView.class);
